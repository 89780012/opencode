...
5. 如果任务明确涉及 JS、EJS 或 Vue 前端源码，前端改动完成后必须运行 `npm run build`，并把构建结果作为实现验证的一部分。
6. 只要前端构建未执行，或构建失败且未说明阻塞，就不能把该任务标记为已完成。
...
- 参数名、字段名和调用顺序与文档一致
- 如果修改了 JS、EJS 或 Vue 前端源码，已运行 `npm run build`
- 如果前端构建失败，已明确记录失败原因、阻塞点和下一步
- 已记录主要约束、风险与文档缺口
