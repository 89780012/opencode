...
- 第一行必须是 `flowchart TD`。
- 节点文案使用简短中文，适合流程图节点。
- 流程图面向不写代码的策略研究、交易和运营人员。源码只用于校验事实，节点必须描述业务条件、交易动作和结果。
- 节点不得出现文件名、函数名、变量名、参数名、枚举名、调用语法，以及 Python、JavaScript、SDK、API 等开发术语。
- 把订阅和回调写成业务触发关系，把计算函数写成指标含义，把接口调用写成读取数据或执行交易的业务动作。
