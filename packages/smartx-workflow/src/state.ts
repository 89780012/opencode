...
export function noteReview(input: { workspace: string; worktree: string; sessionID: string }) {
  return [
    ...
    "6. 只有 `smartx_save_review.items` 中所有检查项的 status 都是 passed，才算审查完全通过；只要任一项是 warning、failed 或 error，都必须按未通过处理。",
    ...
    "9. 第 3 轮审查全部 passed 时，保存后进入后续分析、流程图和调试流程；第 3 轮仍存在非 passed 检查项时，也必须先保存该轮缺陷结果，再完成最后一次修复，然后给出最终结论，不再自动发起第 4 次复审。",
    "10. 审查全部 passed 并保存后，工作流会继续要求重新分析当前代码、重新生成并保存策略逻辑流程图，然后才调用 `smartx_start` 开始调试；主 agent 必须按后续系统提示执行。",
