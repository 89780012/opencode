...
export function buildTemplatePrompt(input: { name: string; type: Exclude<StrategyType, "smartx"> }) {
  const label = input.type === "python" ? "Python" : input.type === "js" ? "JavaScript" : "其他"
  const entry = input.type === "python" ? "main.py" : input.type === "js" ? "index.js" : "README.md"
  const focus =
    input.type === "python"
      ? "请基于当前 Python 模板完成一个可继续迭代的策略脚手架，先解释结构，再直接开始完善代码。"
      : input.type === "js"
        ? "请基于当前 JavaScript 模板完成一个可继续迭代的策略脚手架，先解释结构，再直接开始完善代码。"
        : "请先阅读当前工作区的现有文件，帮我整理结构、目标和下一步改造计划，再按需补充文件。"
