...
export function starter(name: string): Omit<WorkflowRuntimeDetail, "id" | "updated_at"> {
  return {
    ...
    nodes: [
      ...
      node("respond", "respond", 640, 20, "回复", "respond"),
      node("plan", "plan", 640, 150, "规划", "smartx-plan"),
      node("execute", "execute", 640, 320, "执行", "strategy"),
      node("check", "check", 940, 320, "检查", "checker"),
      node("end", "end", 1240, 220, "结束"),
    ],
    edges: [
      edge("e-start-router", "start", "router", "always", "进入路由"),
      edge("e-router-respond", "router", "respond", "respond", "直接回复"),
      ...
      edge("e-respond-end", "respond", "end", "always", "回复完成"),
