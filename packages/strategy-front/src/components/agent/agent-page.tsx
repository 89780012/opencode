...
export function AgentPage() {
  ...
  const reload = useCallback(
    async (spin: boolean = true) => {
      if (spin) {
        setLoad(true);
      }
      setErr("");

      // 运行时列表依赖当前工作区，文件列表只读取全局目录。
      const jobs = [
        path ? agentApi.listRuntime(path) : Promise.resolve<RuntimeAgent[]>([]),
        agentApi.listGlobal(),
      ] as const;
      const [run, cfg] = await Promise.allSettled(jobs);
      const msg: string[] = [];
      ...
      if (run.status === "fulfilled") {
        ...
        setRun([]);
        msg.push(`当前生效 agent 列表加载失败：${note(run.reason, "请求失败")}`);
      }
    ...
    <div className="bg-background h-full overflow-auto">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-6">
          ...
          <CardHeader className="border-b">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div className="space-y-2">
                ...
                <CardDescription>
                  当前页分成两部分：一部分展示当前工作区实际生效的 agent 列表，另一部分管理
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">
          <div>
            全局 agent 文件路径：
            <code>{cfg.root || "~/.config/opencode/agents"}</code>
          </div>
          <div>
            每个自定义 agent 都会写入
            <code>{` <name>.md`}</code>。
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              这里只管理写入全局配置目录的 Markdown agent 文件。
            </p>
          </div>
          {load ? (
        ...
        <section className="space-y-4">
          <div>
            <h2 className="text-lg font-semibold">当前工作区生效 Agent 列表</h2>
            <p className="text-muted-foreground mt-1 text-sm">
              这里直接展示 opencode <code>/agent</code> 返回的当前工作区运行时结果。
            </p>
          </div>
          {!path ? (
            <div className="text-muted-foreground rounded-xl border border-dashed px-4 py-6 text-sm">
              请先在对话页选择一个工作区，再查看该工作区当前生效的 agent 列表。
            </div>
          ) : load ? (
            <div className="text-muted-foreground flex items-center gap-2 text-sm">
              <Loader2 className="size-4 animate-spin" />
