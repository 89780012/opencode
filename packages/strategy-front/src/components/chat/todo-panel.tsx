...
export function TodoPanel(props: Props) {
  ...
  return (
    <div
      ...
      className={cn(
        "embed-todo overflow-hidden rounded-[22px] border border-black/8 bg-slate-50 transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] dark:border-white/10 dark:bg-[#151918]",
        collapsed ? "shadow-none" : "shadow-sm shadow-black/5 dark:shadow-black/20",
      )}
    >
      <div className={cn("embed-todo-head flex items-center gap-3", props.compact ? "px-3 py-2" : "px-3 py-2.5")}>
        <div className="embed-todo-mark flex size-8 shrink-0 items-center justify-center rounded-2xl bg-background/90 text-muted-foreground ring-1 ring-black/6 dark:bg-white/[0.05] dark:ring-white/10">
          <ListTodo className={props.compact ? "size-[14px]" : "size-[15px]"} />
        </div>
        <button
          type="button"
          className="embed-todo-toggle min-w-0 flex-1 text-left"
          onClick={() => {
            setUser((value) => !(value ?? base))
          }}
        >
          <div className="embed-todo-title flex items-center gap-2 text-sm font-medium leading-none">
            <span>待办</span>
            <span className="embed-todo-count rounded-full bg-background/90 px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground ring-1 ring-black/6 dark:bg-white/[0.05] dark:ring-white/10">
              {count} / {total}
            </span>
          </div>
          <div
            className={cn(
              "embed-todo-copy relative overflow-hidden text-muted-foreground",
              props.compact ? "mt-0.5 h-4 text-[11px]" : "mt-1 h-4 text-[12px]",
            )}
          >
            <div
              className={cn(
                "embed-todo-preview absolute inset-0 truncate transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
            ...
            <div
              className={cn(
                "embed-todo-total absolute inset-0 truncate transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          className="embed-todo-arrow size-7 shrink-0 rounded-full text-muted-foreground hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
          onClick={(e) => {
            e.stopPropagation()
            setUser((value) => !(value ?? base))
          }}
      ...
      <div
        className={cn(
          "embed-todo-shell grid overflow-hidden transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)]",
      ...
      >
        ...
        >
          <div className={cn("embed-todo-list", props.compact ? "space-y-1" : "space-y-1.5")}>
            {props.todos.map((item, i) => (
              <div
                ...
                className={cn(
                  "embed-todo-row flex items-start rounded-xl",
                  props.compact ? "gap-2 px-1 py-0.5" : "gap-2.5 px-1.5 py-1",
                )}
              >
          ...
          <div
            className={cn(
              "embed-todo-fade pointer-events-none absolute inset-x-0 top-0 h-4 bg-gradient-to-b from-background to-transparent transition-opacity duration-200",
