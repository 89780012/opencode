      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {props.req.patterns.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {props.req.patterns.map((item) => (
                <code key={item} className="rounded-md border border-amber-200 bg-white/80 px-2 py-1 text-xs text-amber-900">
                  {item}
                </code>
              ))}
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button type="button" variant="outline" onClick={props.onReject} disabled={props.sending}>
              拒绝
            </Button>
            <Button type="button" variant="outline" onClick={() => props.onAllow("always")} disabled={props.sending}>
              始终允许
            </Button>
            <Button type="button" onClick={() => props.onAllow("once")} disabled={props.sending}>
              仅本次允许
            </Button>
          </div>
        </div>
