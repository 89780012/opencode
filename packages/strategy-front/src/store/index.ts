...
import type { ChatFileDiff, ChatMessageInfo, ChatPart, ChatQuestionRequest, ChatTodo, PermissionRequest } from "@/types/chat";
...
export function selectSessionPermissionRequest(
  state: RootState,
  ...
  return sessionPermissionRequest(
    selectWorkspaceSessions(state, workspace),
    selectPermissionRequests(state),
    sessionId,
  );
}

export function selectSessionQuestionRequest(
  state: RootState,
  workspace?: string | null,
  sessionId?: string | null,
) {
