...
- 保存成功后只写入当前会话的“待重审”前端状态，不从 PUT 接口直接启动 AI。主区域在顶栏下方显示紧凑横条，包含状态说明和“重新审查并修改代码”命令。
- 横条操作复用 `StageView` 已有的 `chat.submit`，先切换到“对话与审查”，再向当前会话发送固定指令；成功后清除标记，失败时保留并允许重试。
...
- 待重审状态独立于会话服务端快照，按 `(workspacePath, sessionId)` 在 Redux 中保存递增版本；迟到的保存响应只标记其原始目标会话，不污染用户后来切换到的会话。
- 点击横条时必须校验工作台活动会话与聊天运行时选中会话一致，并复用聊天提交中的 single-flight 状态；失败或会话错位不得提前清除标记。发送成功时只清除点击时捕获的版本，若期间再次保存则保留新版本提示。
- 用户可主动关闭当前版本提示；状态只覆盖当前页面生命周期，会话切换不丢失，硬刷新后自然清空。

## 兼容与回滚

- 不修改 SQLite schema，已有数据库无需迁移。
- 不修改创建会话、列出会话、MCP `get_requirements` 或 `session.update` 标题更新的既有契约。
- 不修改 `PUT /api/workbench/requirements` 契约，也不新增后端待重审字段；刷新页面后的提示持久化不在本次 MVP 范围内。
