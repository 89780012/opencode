...
- 两个入口共用活动任务状态和发起动作。
- 无回测记录时显示“运行回测”和未运行空状态；只有已有完成或失败记录时显示“重新运行”或“重试”。
- 流程图入口在活动期间变为查看进度，不重复提交。
- 回测面板增加 pending/running/done/failed 分支；pending/running 必须在报告区随 Socket 状态显示真实百分比、进度条、耗时和更新时间。
- 修复 `0%` 显示和 failed 被映射成 done 的问题。

## 5. 测试与质量门禁

- 后端测试：状态迁移、空 btId、进度单调、瞬时错误重试、终态停止、幂等命中、活动冲突、重启恢复、先落库后事件。
- 前端测试：会话隔离、乱序事件、重连对账、历史选择不被抢占、重复点击只提交一次、首次运行按钮、`0%` 与失败态。
