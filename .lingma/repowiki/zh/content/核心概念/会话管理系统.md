...
- [packages/ui/src/components/session-turn.tsx](file://packages/ui/src/components/session-turn.tsx)
- [packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx](file://packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx)
- [packages/strategy-front/src/components/workbench/styles/session/session-chat.module.css](file://packages/strategy-front/src/components/workbench/styles/session/session-chat.module.css)
- [packages/web/src/components/Share.tsx](file://packages/web/src/components/Share.tsx)
- [packages/opencode/src/cli/cmd/tui/routes/session/index.tsx](file://packages/opencode/src/cli/cmd/tui/routes/session/index.tsx)
</cite>
...
- 新增了上下文压缩消息显示功能的详细说明
- 增强了合成消息检测系统的文档描述
- 添加了专门的compaction消息渲染逻辑和CSS样式的说明
- 完善了用户对上下文压缩过程的透明度和体验说明
...
**更新** 增强了上下文压缩消息显示功能，新增了专门的compaction消息渲染逻辑和对应的CSS样式，改进了合成消息检测系统，提升了用户对上下文压缩过程的透明度和体验。
...
F["消息部件渲染<br/>message-part.tsx"]
G["会话转轮组件<br/>session-turn.tsx"]
H["消息列表组件<br/>session-message-list.tsx"]
I["共享消息组件<br/>Share.tsx"]
J["TUI会话界面<br/>tui/session/index.tsx"]
end
subgraph "会话内核"
K["消息与部件模型<br/>message-v2.ts"]
L["会话压缩与溢出处理<br/>compaction.ts"]
M["标识符与模式<br/>schema.ts"]
end
subgraph "后端服务"
N["会话Socket接口<br/>session_socket.go"]
end
A --> B
A --> C
A --> D
...
- [packages/ui/src/components/message-part.tsx:1303-1306](file://packages/ui/src/components/message-part.tsx#L1303-L1306)
- [packages/ui/src/components/session-turn.tsx:284-288](file://packages/ui/src/components/session-turn.tsx#L284-L288)
- [packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx:471-478](file://packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx#L471-L478)
- [packages/web/src/components/Share.tsx:350-367](file://packages/web/src/components/Share.tsx#L350-L367)
- [packages/opencode/src/cli/cmd/tui/routes/session/index.tsx:1314-1322](file://packages/opencode/src/cli/cmd/tui/routes/session/index.tsx#L1314-L1322)
...
- 上下文压缩消息显示：新增专门的compaction消息渲染逻辑，提供视觉分隔线和状态提示，增强用户对压缩过程的感知。
- 合成消息检测系统：改进的消息过滤机制，能够识别并处理合成消息，确保显示的准确性。
...
UI->>Kernel : 触发压缩/继续
Kernel-->>UI : 发布事件/更新消息
Note over UI,Kernel : 新增上下文压缩消息显示功能
UI->>UI : 渲染compaction消息分隔线
UI->>UI : 显示压缩状态提示
```
...
- 合成消息处理：新增合成消息检测机制，能够识别并过滤合成内容，确保显示的准确性。
...
- **新增** 上下文压缩消息显示：当检测到上下文压缩时，系统会渲染专门的compaction消息分隔线，提供视觉提示和状态信息。
...
Inject --> ShowCompaction["渲染compaction消息分隔线"]
ShowCompaction --> Finish["标记错误或停止"]
Continue --> Proceed
```
...
### 上下文压缩消息显示系统
- **新增功能** 专门的compaction消息渲染逻辑：当系统检测到上下文压缩时，会在消息列表中插入专门的compaction消息分隔线。
- **视觉设计** 使用border属性创建顶部边框，标题显示"Compaction"，提供清晰的视觉分隔效果。
- **状态提示** 在compaction消息中显示"上下文已压缩"的中文提示，如果是因为上下文溢出导致的压缩，还会显示具体的压缩原因。
- **多平台支持** 支持Web、桌面应用和TUI三种界面风格的一致显示效果。
- **CSS样式** 通过`.compaction`类名提供统一的样式定义，包括::before和::after伪元素的装饰效果。

```mermaid
flowchart TD
CompactionCheck["检测上下文压缩"] --> CreateDiv["创建compaction div元素"]
CreateDiv --> SetClass["设置className='compaction'"]
SetClass --> AddContent["添加中文提示内容"]
AddContent --> ApplyCSS["应用compaction CSS样式"]
ApplyCSS --> RenderBorder["渲染顶部边框"]
RenderBorder --> ShowMessage["显示压缩状态提示"]
ShowMessage --> UserNotice["用户获得压缩过程反馈"]
```

**图表来源**
- [packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx:471-478](file://packages/strategy-front/src/components/workbench/features/session/session-message-list.tsx#L471-L478)
- [packages/strategy-front/src/components/workbench/styles/session/session-chat.module.css:1116-1133](file://packages/strategy-front/src/components/workbench/styles/session/session-chat.module.css#L1116-L1133)
- [packages/opencode/src/cli/cmd/tui/routes/session/index.tsx:1314-1322](file://packages/opencode/src/cli/cmd/tui/routes/session/index.tsx#L1314-L1322)
...
### 合成消息检测系统
- **改进机制** 新增合成消息检测逻辑，能够识别并过滤合成内容，确保显示的准确性。
- **过滤规则** 在消息渲染过程中，系统会过滤掉具有`synthetic: true`标记的文本部件，防止合成内容被显示给用户。
- **多平台一致性** 合成消息检测在Web、桌面应用和TUI界面中保持一致的行为。
- **调试支持** 在Share组件中提供了调试模式，可以查看完整的消息结构，包括合成标记。

```mermaid
flowchart TD
FilterStart["开始过滤"] --> CheckType["检查部件类型"]
CheckType --> IsText{"是否为text类型?"}
IsText --> |否| KeepPart["保留部件"]
IsText --> |是| CheckSynthetic["检查synthetic标记"]
CheckSynthetic --> IsSynthetic{"synthetic === true?"}
IsSynthetic --> |是| SkipPart["跳过此部件"]
IsSynthetic --> |否| KeepPart
SkipPart --> NextPart["处理下一个部件"]
KeepPart --> NextPart
NextPart --> FilterEnd["过滤完成"]
```
...
- **新增** 合成消息过滤：通过高效的过滤机制避免渲染不必要的合成内容，提升渲染性能。
...
- **新增** 上下文压缩显示问题：如果compaction消息没有正确显示，检查CSS样式文件和消息渲染逻辑，确认compaction部件的正确渲染。
...
## 结论
该会话管理系统通过清晰的分层设计与完善的内核能力，实现了高可用的历史窗口管理、稳健的上下文压缩与溢出处理、可靠的持久化与恢复机制。配合工作流与模型绑定规则，系统在复杂多智能体协作场景中保持语义正确与用户体验稳定。**更新** 增强的上下文压缩消息显示功能和改进的合成消息检测系统进一步提升了用户的透明度和体验。建议在生产环境中结合监控指标与日志，持续优化压缩策略与缓存命中率。
...
- **新增** 利用上下文压缩提示：当看到"上下文已压缩"的提示时，理解系统正在进行的优化操作，有助于更好地管理长对话的上下文。
- **新增** 合成消息过滤：系统会自动过滤合成内容，用户无需担心看到不相关的中间结果。
