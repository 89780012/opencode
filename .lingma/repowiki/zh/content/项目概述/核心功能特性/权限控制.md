...
- [strategy-flowchart-generator.md](file://packages/smartx-workflow/agents/strategy-flowchart-generator.md)
- [smartx-helper.md](file://packages/strategy-service/internal/asset/workspace/agents/smartx-helper.md)
</cite>

## 更新摘要
**变更内容**
- 新增外部目录访问权限配置示例，展示AI代理对外部资源的扩展访问能力
- 更新代理权限配置章节，包含具体代理的外部目录访问配置案例
- 增强权限配置最佳实践，涵盖AI代理的外部资源访问场景
...
**更新** 新增AI代理的外部目录访问配置示例，展示智能工作流代理对外部资源的扩展访问能力
...
### AI代理外部目录访问配置
**新增** 基于实际代码变更，展示AI代理如何配置外部目录访问权限

OpenCode中的AI代理（如策略流程图生成器和SmartX助手）通过Markdown配置文件定义权限规则。这些代理通常需要访问项目外部的资源来进行完整的分析和实现。

#### 策略流程图生成器代理
该代理配置展示了完整的外部目录访问权限设置：
...
这些配置表明：
- 代理默认具有较高的权限灵活性，特别是外部目录访问
- 通过 "*" 通配符允许访问所有外部路径
- 代理权限通常比默认配置更宽松，以支持复杂的开发工作流

**章节来源**
- [strategy-flowchart-generator.md:16-17](file://packages/smartx-workflow/agents/strategy-flowchart-generator.md#L16-L17)
- [smartx-helper.md:19-20](file://packages/strategy-service/internal/asset/workspace/agents/smartx-helper.md#L19-L20)
...
**更新** 新增的AI代理外部目录访问配置展示了OpenCode在智能工作流场景下的权限设计思路，通过合理的权限配置平衡了安全性与功能性需求。
...
**更新** 基于新增的AI代理配置，补充以下最佳实践：

- **AI代理权限设计**：智能工作流代理通常需要更宽松的外部目录访问权限，可通过 "*" 通配符简化配置
- **权限层次化**：用户权限应比代理权限更严格，通过代理权限实现功能性的平衡
- **最小权限原则**：即使代理配置了外部目录访问，也应在具体工具层面实施更精细的限制
- **定期审计**：对于具有广泛外部访问权限的代理，应建立定期的权限审计机制
